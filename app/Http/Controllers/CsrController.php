...
class CsrController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($data['approval_status'] === CsrPengajuan::STATUS_PENDING_NOC) {
            ...
            $nomorText = $csr->nomor ? " ({$csr->nomor})" : "";
            Notification::send($nocLeader, new CsrApprovalNotification($csr, "Pengajuan CSR baru{$nomorText} dari " . Auth::user()->name . " menunggu persetujuan NOC Leader."));
            ...
            $nomorText = $csr->nomor ? " ({$csr->nomor})" : "";
            Notification::send($notifiableUsers, new CsrApprovalNotification($csr, "Pengajuan CSR baru{$nomorText} telah dibuat oleh " . Auth::user()->name . ". Menunggu persetujuan Anda."));
    ...
    public function approve(Request $request, $id)
    {
        ...
        if ($pemohon) {
            $nomorText = $csr->nomor ? " ({$csr->nomor})" : "";
            $pemohon->notify(new CsrApprovalNotification($csr, "Pengajuan CSR Anda{$nomorText} telah disetujui oleh {$who}."));
        }

        // Notify Next Approver
        $nextRole = null;
        if ($newStatus === CsrPengajuan::STATUS_PENDING)
            $nextRole = 'manager';
        if ($newStatus === CsrPengajuan::STATUS_APPROVED_MANAGER)
            $nextRole = 'accounting';
        if ($newStatus === CsrPengajuan::STATUS_APPROVED_ACCOUNTING)
            $nextRole = 'direktur';
        if ($newStatus === CsrPengajuan::STATUS_APPROVED_DIREKTUR)
            $nextRole = 'penasihat';

        if ($nextRole) {
            $nextUsers = User::whereIn('role', [$nextRole, 'superadmin'])->get();
            $nomorText = $csr->nomor ? " ({$csr->nomor})" : "";
            Notification::send($nextUsers, new CsrApprovalNotification($csr, "Pengajuan CSR{$nomorText} telah disetujui oleh {$who} dan menunggu persetujuan Anda."));
    ...
    public function reject(Request $request, $id)
    {
        ...
        if ($pemohon) {
            $nomorText = $csr->nomor ? " ({$csr->nomor})" : "";
            $pemohon->notify(new CsrApprovalNotification($csr, "❌ Pengajuan CSR Anda{$nomorText} DITOLAK oleh {$rejectorLabel}. Alasan: " . ($request->rejection_reason ?: '-')));
        }
    ...
    public function updateAccountingNotes(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'bukti_dp'          => 'nullable|image|max:5120',
        ...
        if ($request->hasFile('bukti_dp')) {
            $updateData['bukti_dp'] = $request->file('bukti_dp')->store('bukti_dp_csr', 'public');
        }

        if ($request->hasFile('bukti_transfer')) {
            $updateData['bukti_transfer'] = $request->file('bukti_transfer')->store('bukti_transfer_csr', 'public');
        }

        $csr->update($updateData);

        return redirect()->back()->with('success', '✅ Catatan, Nomor Surat, dan Pembayaran berhasil disimpan.');
    }
}
