...
<body>
    ...
    <div style="padding: 20px 30px 30px 30px;">
        <style>
            ...
            .stat-card-gradient {
                background: linear-gradient(135deg, #071152 0%, #1a365d 100%);
            }
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 mt-4">
            ...
            <div class="stat-card-gradient rounded-3xl p-6 text-white shadow-xl relative overflow-hidden group">
                <div
                    class="absolute -right-6 -top-6 w-32 h-32 bg-white opacity-10 rounded-full blur-2xl group-hover:scale-110 transition-transform duration-500">
                </div>
                <div class="flex justify-between items-start mb-4 relative z-10">
                    <div>
                        <p class="text-blue-100 font-medium text-sm tracking-wide uppercase mb-1">Total Pencapaian
                            (DONE)</p>
            ...
            <div class="stat-card-gradient-2 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden group">
                ...
                <div class="w-full bg-white/20 rounded-full h-1.5 mt-8 mb-2 relative z-10">
                    <div class="bg-white h-1.5 rounded-full"
                        style="width: {{ min(round(($bmnCount / 240) * 100), 100) }}%"></div>
                </div>
                <div class="text-xs text-sky-100 font-medium text-right relative z-10">{{ $bmnCount }}/240 &mdash;
                    {{ min(round(($bmnCount / 240) * 100), 100) }}% dari Target</div>
            </div>
            <!-- SL Card -->
            <div class="stat-card-gradient-3 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden group">
                ...
                <div class="flex justify-between items-start mb-4 relative z-10">
                    <div>
                        <p class="text-emerald-100 font-medium text-sm tracking-wide uppercase mb-1">Target SL Selesai
                        </p>
                ...
                <div class="w-full bg-white/20 rounded-full h-1.5 mt-8 mb-2 relative z-10">
                    <div class="bg-white h-1.5 rounded-full"
                        style="width: {{ min(round(($slCount / 121) * 100), 100) }}%"></div>
                </div>
                <div class="text-xs text-emerald-100 font-medium text-right relative z-10">{{ $slCount }}/121 &mdash;
                    {{ min(round(($slCount / 121) * 100), 100) }}% dari Target</div>
            </div>
        </div>
                ...
                <div class="flex justify-between items-center mb-4 flex-wrap gap-3">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <div class="flex rounded-lg overflow-hidden border border-slate-200 text-xs font-semibold">
                            <button id="btn-mode-bulan" onclick="setFilterMode('bulan')"
                                class="px-3 py-1.5 bg-[#071152] text-white">Per Bulan</button>
                            <button id="btn-mode-range" onclick="setFilterMode('range')"
                                class="px-3 py-1.5 bg-white text-slate-600 hover:bg-slate-50">Rentang Tanggal</button>
                        </div>
                        <!-- Filter: Per Bulan -->
                        <div id="filter-bulan" class="flex items-center gap-2">
                            ...
                            <button onclick="applyChartFilter()"
                                class="text-xs bg-[#071152] text-white px-3 py-1.5 rounded-lg hover:bg-blue-900">Tampilkan</button>
                        </div>
            ...
            <div class="glass-panel p-6 rounded-3xl flex flex-col">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="relative group cursor-pointer">
                        ...
                        <div
                            class="absolute right-0 w-48 p-2 bg-slate-800 text-white text-xs rounded-lg mt-2 hidden group-hover:block z-50 shadow-lg top-full">
                            Total komulatif per bulan yang statusnya sudah DONE.</div>
                    </div>
                </div>
                    ...
                    <div class="space-y-3">
                        ...
                        @foreach($monthsOrder as $month)
                            ...
                            @if($t > 0 || $month == now()->locale('id')->monthName)
                                <div
                                    class="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm hover:shadow-md transition-shadow relative overflow-hidden {{ $month == now()->locale('id')->monthName ? 'ring-2 ring-blue-100 bg-blue-50/20' : '' }}">
                                    @if($month == now()->locale('id')->monthName)
                                        <div
                                            class="absolute right-0 top-0 bg-[#071152] text-white text-[10px] font-bold px-2 py-1 rounded-bl-lg">
                                            BULAN INI</div>
                                    @endif
                                    ...
                                    <div class="flex gap-2">
                                        <div
                                            class="flex-1 bg-gradient-to-r from-sky-50 to-blue-50 rounded-lg py-1.5 px-2 text-center border border-sky-100/50">
                                            <div class="text-[10px] text-sky-600 font-bold uppercase mb-0.5">BMN</div>
                                            <div class="text-sm font-semibold text-slate-700">{{ $b }}</div>
                                        </div>
                                        <div
                                            class="flex-1 bg-gradient-to-r from-emerald-50 to-green-50 rounded-lg py-1.5 px-2 text-center border border-emerald-100/50">
        ...
        <div class="glass-panel p-6 rounded-3xl">
            <div class="flex justify-between items-center mb-6">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 shadow-inner">
                ...
                <a href="{{ url('/PMLiberta') }}"
                    class="flex items-center gap-1.5 text-sm font-semibold text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-4 py-2 rounded-xl transition-all">
            ...
            <div id="sites-grid-container">
                @forelse($sites ?? [] as $index => $site)
                    ...
                    @endif
                    <div
                        class="group relative bg-white border border-slate-100 rounded-2xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 overflow-hidden">
                        {{-- Accent stripe --}}
                        <div
                            class="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl {{ $isBMN ? 'bg-sky-400' : 'bg-emerald-400' }}">
                        </div>
                        <div class="flex items-start justify-between pl-3">
                            ...
                            <div class="flex items-start gap-3 flex-1 min-w-0">
                                {{-- Index badge --}}
                                <div
                                    class="w-8 h-8 rounded-xl {{ $isBMN ? 'bg-sky-50 text-sky-600' : 'bg-emerald-50 text-emerald-600' }} flex items-center justify-center font-bold text-xs flex-shrink-0">
                                    {{ $index + 1 }}
                                </div>
                                <div class="min-w-0 flex-1">
                                    {{-- Site ID --}}
                                    <span
                                        class="inline-block font-mono text-[10px] font-semibold {{ $isBMN ? 'text-sky-700 bg-sky-50' : 'text-emerald-700 bg-emerald-50' }} px-2 py-0.5 rounded mb-1">
                                        {{ $site->site_id }}
                                    </span>
                                    {{-- Nama Lokasi --}}
                                    <p class="font-bold text-slate-800 text-sm leading-tight truncate group-hover:text-[#071152] transition-colors"
                                        title="{{ $site->nama_lokasi }}">
                                        {{ $site->nama_lokasi }}
                                    </p>
                                    {{-- Lokasi --}}
                                    <div class="flex items-center gap-1 mt-1 text-slate-400 text-xs">
                                        ...
                                        <span class="truncate"
                                            title="{{ strtoupper($site->kabupaten) }}">{{ strtoupper($site->kabupaten) }}</span>
                                    </div>
                                </div>
                            </div>
                            {{-- Right: badges --}}
                            <div class="flex flex-col items-end gap-1.5 flex-shrink-0 ml-2">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-600 border border-slate-200">
                                        {{ $status ?: 'N/A' }}
                                    </span>
                                @endif
                                {{-- Kategori badge --}}
                                <span
                                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold {{ $isBMN ? 'bg-sky-100 text-sky-700' : 'bg-emerald-100 text-emerald-700' }}">
                                    <span
                                        class="w-1.5 h-1.5 rounded-full {{ $isBMN ? 'bg-sky-500' : 'bg-emerald-500' }}"></span>
                                    {{ $isBMN ? 'BMN' : 'SL' }}
                                </span>
                                {{-- Bulan --}}
