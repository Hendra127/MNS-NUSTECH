...
<div class="pt-32 pb-20 bg-slate-50 min-h-screen">
  <div class="max-w-[95%] 2xl:max-w-[80%] mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-8">
        <a href="{{ route('news.index') }}" class="inline-flex items-center text-sm font-bold text-slate-500 hover:text-sky-600 transition-colors bg-white px-4 py-2 rounded-full border border-slate-200 shadow-sm hover:shadow-md"><i class="fa-solid fa-arrow-left mr-2"></i> Kembali ke Daftar Berita</a>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-8 sm:p-10">
                    <div class="flex items-center gap-4 mb-4">
                        <span class="text-xs text-sky-600 font-bold bg-sky-50 px-3 py-1.5 rounded-full"><i class="fa-regular fa-calendar mr-1"></i> {{ $news->published_at ? $news->published_at->format('d M Y') : '' }}</span>
                        <span class="text-xs text-slate-400 font-semibold"><i class="fa-solid fa-tag mr-1"></i> Berita Umum</span>
                    </div>
                    <h1 class="text-3xl sm:text-4xl font-heading font-black text-slate-900 mb-8 leading-tight">{{ $news->title }}</h1>
                    
                    <div class="prose prose-slate prose-sky max-w-none prose-p:leading-relaxed prose-p:text-slate-600 prose-a:text-sky-600 hover:prose-a:text-sky-700">
                        {!! nl2br(e($news->caption)) !!}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sticky top-28">
                <h3 class="text-xl font-heading font-black text-slate-800 mb-6 flex items-center gap-2">
                    <span class="w-8 h-8 rounded-lg bg-sky-100 text-sky-600 flex items-center justify-center"><i class="fa-solid fa-clock-rotate-left"></i></span> Berita Terbaru
                </h3>
                <div class="flex flex-col gap-5">
                    @forelse($recentNews as $recent)
                        <a href="{{ route('news.show', $recent->id) }}" class="group flex gap-4 items-start">
                            <div class="w-20 h-20 rounded-xl overflow-hidden bg-slate-100 shrink-0 border border-slate-100">
                                @if($recent->image_path)
                                    <img src="{{ asset($recent->image_path) }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                @else
                                    <div class="w-full h-full bg-sky-50 flex items-center justify-center text-sky-400">
                                        <i class="fa-regular fa-newspaper text-2xl"></i>
                                    </div>
                                @endif
                            </div>
                            <div class="flex-1 min-w-0">
                                <h4 class="font-bold text-slate-800 text-sm leading-snug line-clamp-2 group-hover:text-sky-600 transition-colors mb-1.5">{{ $recent->title }}</h4>
                                <span class="text-[10px] text-slate-500 flex items-center gap-1"><i class="fa-regular fa-calendar text-sky-400"></i> {{ $recent->published_at ? $recent->published_at->format('d M Y') : '' }}</span>
                            </div>
                        </a>
                    @empty
