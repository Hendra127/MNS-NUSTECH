    ...
    <div class="content-container">
        <div class="card-header d-flex flex-column flex-lg-row justify-content-between align-items-start align-items-lg-center gap-3"
            ...
            <div class="actions flex-shrink-0">
                @if(auth()->check() && auth()->user()->hasAdminAccess())
                    <!-- tombol plus (Add) -->
                    <button type="button" class="btn-action bi bi-plus" data-bs-toggle="modal"
                        data-bs-target="#modalLaporanPM" title="Tambah Data">
                    </button>
                    <!-- tombol upload (Import) -->
                    <button type="button" class="btn-action bi bi-upload" title="Upload Data" data-bs-toggle="modal"
                        data-bs-target="#modalImportLaporan">
                    </button>
                @endif
                <!-- tombol download (Export) -->
                <a href="{{ route('laporancm.export') }}" class="btn-action bi bi-download" title="Download Excel"
                    ...
                    <div class="col-12 col-md-auto">
                        <div class="search-box d-flex align-items-center w-100">
                            <input type="text" name="search" id="searchInput" placeholder="Search..."
                                ...
                                style="flex-grow: 1; border: none; outline: none; background: transparent; padding-left: 15px;">
                ...
                <tbody>
                    @forelse($cm_data as $item)
                        <tr class="text-center">
                            ...
                            <td>{{ $item->notes ?? '-' }}</td>
                            <td>{{ $item->biaya_teknisi ? 'Rp ' . number_format($item->biaya_teknisi, 0, ',', '.') : '-' }}
                            </td>
                            <td>
                                @if($item->foto_on_site)
                                    <a href="{{ Storage::url($item->foto_on_site) }}" target="_blank"
                                        class="btn btn-sm btn-info text-white"><i class="bi bi-image"></i> Lihat</a>
                                @else
                                    -
                                @endif
                            </td>
                            <td>
                                @if($item->bukti_transfer)
                                    <a href="{{ Storage::url($item->bukti_transfer) }}" target="_blank"
                                        class="btn btn-sm btn-info text-white"><i class="bi bi-file-earmark-text"></i> Lihat</a>
                                @else
                                    -
                                @endif
                            </td>
                            @if(auth()->check() && auth()->user()->hasAdminAccess())
                                <td class="text-center sticky-col-right">
                                    <button class="btn btn-sm btn-edit" data-bs-toggle="modal"
                                        data-bs-target="#modalEditLaporanPM" data-id="{{ $item->id }}"
                                        data-tanggal_on_site="{{ $item->tanggal_on_site }}" data-site_id="{{ $item->site_id }}"
                                        data-nama_site="{{ $item->nama_site }}" data-nama_teknisi="{{ $item->nama_teknisi }}"
                                        data-laporan_cm="{{ $item->laporan_cm }}" data-notes="{{ $item->notes }}"
                                    ...
                                    <form id="delete-form-{{ $item->id }}" action="{{ route('laporancm.destroy', $item->id) }}"
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="tanggal_on_site" id="tanggal_on_site"
                                    class="form-control form-control-lg rounded-3" value="{{ date('Y-m-d') }}" required>
                            </div>
                            <!-- Nama Site -->
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Nama Site</label>
                                <select name="site_id" id="siteSelect"
                                    class="form-select form-select-lg rounded-3 select2-enable" required>
                                    <option value="">Pilih atau cari Nama Site</option>
                                    @foreach($sites as $s)
                                        <option value="{{ $s->site_id }}" data-siteid="{{ $s->site_id }}"
                                            data-lokasi="{{ $s->sitename }}" data-kab="{{ $s->kabupaten ?? $s->kab ?? '' }}"
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" id="siteIdView"
                                    class="form-control form-control-lg rounded-3 bg-light" placeholder="Site ID"
                                    readonly>
                            </div>
                            <!-- Nama Site auto -->
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="nama_teknisi" class="form-control form-control-lg rounded-3"
                                    placeholder="Nama Teknisi" required>
                            </div>
                            <!-- Laporan CM/PM -->
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Correctiv Maintenance/PM</label>
                                <input type="text" name="laporan_cm" class="form-control form-control-lg rounded-3"
                                    placeholder="Nomor atau Deskripsi Laporan">
                            </div>
                            <!-- Notes -->
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Notes</label>
                                <input type="text" name="notes" class="form-control form-control-lg rounded-3"
                                    placeholder="Notes atau Kendala">
                            </div>
                            <!-- Biaya Teknisi -->
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Biaya Teknisi</label>
                                <input type="number" name="biaya_teknisi" class="form-control form-control-lg rounded-3"
                                    placeholder="Contoh: 150000">
                            </div>
                            <!-- Foto On Site (file upload) -->
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Foto On Site</label>
                                <input type="file" name="foto_on_site" accept="image/*,application/pdf"
                ...
                <div class="modal-body px-4 pb-4">
                    <form id="formEditLaporan" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Site ID</label>
                                <input type="text" id="edit_site_id" name="site_id" class="form-control bg-light"
                                    readonly>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Nama Site</label>
                                <input type="text" id="edit_nama_site" name="nama_site" class="form-control bg-light"
                                    readonly>
                            </div>
                            <hr class="my-3">
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Tanggal On Site</label>
                                <input type="date" name="tanggal_on_site" id="edit_tanggal_on_site"
                                    class="form-control form-control-lg rounded-3" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Nama Teknisi</label>
                                <input type="text" name="nama_teknisi" id="edit_nama_teknisi"
                                    class="form-control form-control-lg rounded-3" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Correctiv Maintenance/PM</label>
                                <input type="text" name="laporan_cm" id="edit_laporan_cm"
                                    class="form-control form-control-lg rounded-3">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Notes</label>
                            ...
                            <div class="col-md-12">
                                <label class="form-label fw-semibold">Biaya Teknisi</label>
                                <input type="number" name="biaya_teknisi" id="edit_biaya_teknisi"
                                    class="form-control form-control-lg rounded-3">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Update Foto On Site (Opsional)</label>
                                <input type="file" name="foto_on_site_update" accept="image/*,application/pdf"
                                    class="form-control form-control-lg rounded-3">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Update Bukti Transfer (Opsional)</label>
                                <input type="file" name="bukti_transfer_update" accept="image/*,application/pdf"
                                    class="form-control form-control-lg rounded-3">
                            </div>
                        </div>
                        <div class="d-flex justify-content-end gap-3 mt-4">
                            <button type="submit" class="btn btn-success btn-lg px-5 rounded-3 shadow-sm"
                                style="background-color: #0c2484 !important;">
                                UPDATE DATA
                            </button>
                            <button type="button" class="btn btn-secondary btn-lg px-5 rounded-3"
                                data-bs-dismiss="modal">
                ...
                <div class="modal-body px-4 pb-4">
                    <form action="{{ route('laporancm.import') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="file" name="file" class="form-control form-control-lg rounded-3"
