...
<head>
    ...
    <style>
        ...
        .stat-card-item:hover { transform: translateY(-3px); box-shadow: 0 6px 20px rgba(0,0,0,0.1); }
        .stat-icon {
        ...
        .stat-number { font-size: 1.6rem; font-weight: 800; color: #1a1a2e; line-height: 1; }
        ...
        .log-table thead th {
            background: #f8f9fa; padding: 12px 16px; font-size: 11px;
            text-transform: uppercase; letter-spacing: 0.8px; color: #666;
            font-weight: 700; border-bottom: 2px solid #eee; white-space: nowrap;
        }
        .log-table tbody td {
        ...
        .user-info .user-name-sm { font-weight: 600; color: #1a1a2e; font-size: 13px; }
        .user-info .user-role-sm { font-size: 10px; color: #999; }

        .badge-page {
            font-size: 10px; padding: 3px 10px; border-radius: 20px;
            font-weight: 600; letter-spacing: 0.3px;
        }
        .badge-page.datasite { background: #e8f4fd; color: #0d6efd; }
        .badge-page.open-ticket { background: #fff3cd; color: #856404; }
        .badge-page.nav-modal { background: #d1e7dd; color: #0f5132; }

        .ip-mono { font-family: 'Courier New', monospace; font-weight: 700; color: #0d6efd; font-size: 13px; }
        .tunnel-badge {
        ...
        .time-sub { font-size: 10px; color: #aaa; display: block; }
        ...
        .search-filter-bar input:focus { border-color: #4facfe; }
        ...
        [data-bs-theme="dark"] .stat-number { color: #e0e0e0 !important; }
        [data-bs-theme="dark"] .stat-label { color: #888 !important; }
        ...
        [data-bs-theme="dark"] .log-table-header h2 { color: #e0e0e0 !important; }
        ...
        [data-bs-theme="dark"] .user-avatar-sm { border-color: #444 !important; }
        [data-bs-theme="dark"] .user-name-sm { color: #e0e0e0 !important; }
        [data-bs-theme="dark"] .user-role-sm { color: #777 !important; }

        [data-bs-theme="dark"] .ip-mono { color: #64b5f6 !important; }
        ...
        [data-bs-theme="dark"] .badge-page.datasite {
            background: rgba(13,110,253,0.15) !important; color: #64b5f6 !important;
        }
        [data-bs-theme="dark"] .badge-page.open-ticket {
            background: rgba(255,193,7,0.15) !important; color: #ffd54f !important;
        }
        [data-bs-theme="dark"] .badge-page.nav-modal {
            background: rgba(25,135,84,0.15) !important; color: #81c784 !important;
        }

        [data-bs-theme="dark"] .time-info { color: #bbb !important; }
        [data-bs-theme="dark"] .time-sub { color: #777 !important; }
        ...
        .badge-status.success { background: #d1e7dd; color: #0f5132; }
        .badge-status.failed  { background: #f8d7da; color: #842029; }
        .badge-status.unknown { background: #e2e3e5; color: #41464b; }

        [data-bs-theme="dark"] .badge-status.success { background: rgba(25,135,84,0.2) !important; color: #81c784 !important; }
        [data-bs-theme="dark"] .badge-status.failed  { background: rgba(220,53,69,0.2) !important;  color: #ef9a9a !important; }
        [data-bs-theme="dark"] .badge-status.unknown { background: rgba(108,117,125,0.2) !important; color: #adb5bd !important; }

        [data-bs-theme="dark"] .empty-state { color: #666 !important; }

        [data-bs-theme="dark"] h1 { color: #e0e0e0 !important; }
        [data-bs-theme="dark"] p { color: #999 !important; }
    </style>
</head>
<body>
...
<div style="max-width: 1200px; margin: 0 auto; padding: 25px 20px;">
    ...
    <div style="margin-bottom: 20px;">
        <h1 style="font-size: 1.5rem; font-weight: 800; color: #1a1a2e; display: flex; align-items: center; gap: 10px;">
    ...
    <div class="stat-cards">
        <div class="stat-card-item">
            ...
            <div>
                <div class="stat-number">{{ $totalToday }} <span style="font-size: 14px; font-weight: 500; opacity: 0.7;">Sesi</span></div>
        ...
        <div class="stat-card-item">
            ...
            <div>
                <div class="stat-number">{{ $totalWeek }} <span style="font-size: 14px; font-weight: 500; opacity: 0.7;">Sesi</span></div>
        ...
        <div class="stat-card-item">
            ...
            <div>
                <div class="stat-number">{{ $totalAll }} <span style="font-size: 14px; font-weight: 500; opacity: 0.7;">Sesi</span></div>
    ...
    <div class="log-table-card">
        <div class="log-table-header">
            ...
            <form method="GET" action="{{ route('remotelog') }}" class="search-filter-bar">
                ...
                <button type="submit" class="btn btn-sm btn-primary" style="border-radius: 8px; padding: 8px 16px; font-size: 13px;">
                    <i class="bi bi-funnel"></i> Filter
                </button>
                <a href="{{ route('remotelog') }}" class="btn btn-sm btn-outline-secondary" style="border-radius: 8px; padding: 8px 12px; font-size: 13px;">
                    Reset
                </a>
            </form>
        </div>
        ...
        <div style="overflow-x: auto;">
            <table class="log-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-center" style="color: #aaa; font-weight: 600;">{{ $logs->firstItem() + $index }}</td>
                        <td>
                            <div class="user-cell">
                                @if($log->user && $log->user->photo)
                                    <img src="{{ asset('storage_public/' . $log->user->photo) }}" class="user-avatar-sm" alt="">
                                @else
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($log->user_name) }}&background=random&size=32" class="user-avatar-sm" alt="">
                                @endif
                                <div class="user-info">
                        ...
                        <td>
                            <div class="time-info">
                                ...
                                <span class="time-sub">{{ \Carbon\Carbon::parse($log->created_at)->timezone('Asia/Makassar')->format('H:i') }} WITA</span>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="9">
                            <div class="empty-state">
                                ...
                                <div style="font-size: 12px; margin-top: 5px;">Log akan tercatat otomatis saat user menggunakan fitur Remote Mikrotik</div>
                            </div>
                        </td>
                    </tr>
                    @endforelse
